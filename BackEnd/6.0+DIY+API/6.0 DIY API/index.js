...
app.get("/filter", (req, res)=>{
  ...
  const foundJoke = jokes.filter((typ) => typ.jokeType === type); 
  res.json(foundJoke);
});
...
app.put("/jokes", (req, res)=>{
  ...
  const sj = jokes.findIndex((joke)=> id === joke.id)
  jokes[sj] = jokePost;
  res.json(jokePost);
});
//6. PATCH a joke
...
app.delete("/jokes/:id", (req,res)=>{
  ...
  const jokeId = jokes.findIndex((joke)=> joke.id === idx)
  if(jokeId > -1){
    ...
    res
      .status(404)
      .json({
        error: `jokes with ${jokeId} not found, No jokes were deleted`
      });
  }
});
//8. DELETE All jokes
app.delete("/all", (req,res)=>{
  ...
  if(userKey === masterKey){
    ...
    res
      .send(404)
      .json({error: `You are not autherized to perform this action`});
  };
});
app.listen(port, () => {
